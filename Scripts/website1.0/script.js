...
function TextSetup()
{
    ...
    elem = document.createElement('div'); // The Div object represents an HTML <div> element
    ...
    function render()
    {
        ...
        mesh_primary[0].updateWorldMatrix(true, false);
        mesh_primary[0].getWorldPosition(tempV);
        ...
        show = intersectedObjects.length && mesh_primary[0] === intersectedObjects[0].object;
        ...
        x = (tempV.x *  .5 + .5) * canvas.clientWidth;
        y = (tempV.y * -.5 + .5) * canvas.clientHeight;

        // move the elem to that position
        elem.style.transform = `translate(-50%, -50%) translate(${x}px,${y}px)`;

        //elem.style.position = 'relative(${pos[0].x}$px)';
